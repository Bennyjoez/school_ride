...
// Stat Card
export function StatCard({ label, value, icon, color = 'blue', linkTo, loading }) {
  const colors = {
...
export function TripStatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${styles[status] ?? styles.scheduled}`}>
      {status === 'active' && (
          ...
          {trips.map(trip => (
            <tr key={trip.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-sm text-gray-500">
                {trip.trip_date ? format(parseISO(trip.trip_date), 'dd MMM yyyy') : '—'}
              </td>
              <td className="px-4 py-3 text-sm text-gray-500">{trip.driver_name ?? '—'}</td>
              <td className="px-4 py-3 text-sm text-gray-500 font-mono text-xs">{trip.vehicle_plate ?? '—'}</td>
