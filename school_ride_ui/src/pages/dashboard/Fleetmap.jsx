...
export function FleetMap({
  ...
  const { data: trips } = useQuery({
    ...
    queryFn: () =>
      getTrips().then((r) => r.data.filter((t) => t.trip_date === today)),
  ...
  useEffect(() => {
    ...
    Object.entries(positions).forEach(([tripId, pos]) => {
      ...
      if (markersRef.current[tripId]?.marker) {
        ...
        markersRef.current[tripId].marker.setLatLng(latlng).setIcon(icon);
  ...
  useEffect(() => {
    ...
    trips.forEach((trip) => {
      ...
      const stopMarkers = stops.map((stop) => {
        ...
        return L.marker([Number(stop.latitude), Number(stop.longitude)], {
          icon: stopIcon,
        })
          .bindPopup(
            `
  ...
  const selectedTrip = selected
    ? trips?.find((t) => String(t.id) === selected)
    : null;
          ...
          >
            ...
            {scheduledTrips.length > 0 && (
              <p className="text-xs text-gray-300 mt-1">
                {scheduledTrips.length} trip
                {scheduledTrips.length > 1 ? "s" : ""} scheduled today
              </p>
            )}
          </div>
        )}
